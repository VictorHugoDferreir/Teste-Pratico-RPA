...
class LeitorPlanilha:
    ...
    def ler(self, caminho_arquivo: Path) -> list[Cliente]:
        ...
        cabecalhos = {
            nome: indice
            for indice, nome in enumerate(next(aba.iter_rows(max_row=1, values_only=True)))
            if nome
        }

        for coluna in Configuracoes.CABECALHOS_OBRIGATORIOS:
            if coluna not in cabecalhos:
                raise ValueError(f"Cabeçalho '{coluna}' não encontrado na planilha.")
